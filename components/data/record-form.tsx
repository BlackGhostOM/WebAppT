...
export function recordToValues(def: EntityDef, record: Record<string, unknown>): Values {
  ...
  for (const f of def.fields) {
    let v = record[f.name];
    if (def.key === "products" && ["supplierCost", "internalCost", "minSellingPrice", "recommendedSellingPrice", "customerSellingPrice"].includes(f.name))
      v = pricing[f.name];
...
function FieldInput({
  ...
  const common = (
    <Label htmlFor={id} className="flex items-center gap-1">
      ...
      {field.sensitive && <span className="rounded-full bg-destructive-soft px-1.5 text-xs font-medium text-destructive-text">{t.data.sensitive}</span>}
    </Label>
  );
    ...
    case "enum":
      control = (
        <Select
          ...
          items={(field.options ?? []).map((o) => ({ value: o, label: labelOf(o, locale) }))}
        >
    ...
    case "date":
      control = (
        <Input
          ...
          type="date"
          dir="ltr"
          value={toDateInputValue(value as number | undefined)}
      ...
      control = (
        <div className="flex gap-2">
          <Input
            ...
            step="0.001"
            min={0}
            value={m.amount === undefined ? "" : String(m.amount)}
            onChange={(e) => onChange(e.target.value === "" ? undefined : { amount: Number(e.target.value), currency: m.currency ?? "OMR" })}
            className="flex-1"
          />
          <Select
            value={m.currency ?? "OMR"}
            onValueChange={(v) => onChange({ amount: m.amount ?? 0, currency: String(v ?? "OMR") })}
      ...
      if (field.options || field.refTable) {
        ...
        control = (
          <Input
            ...
            placeholder={t.data.tagsPlaceholder}
          />
        );
      }
      break;
  ...
  return (
    <div
      className={cn(
        ...
        error && "[&_[data-slot=input]]:border-destructive [&_[data-slot=select-trigger]]:border-destructive [&_[data-slot=textarea]]:border-destructive",
      )}
    >
      {common}
      {control}
      {field.helpAr && locale === "ar" && !error && <p className="text-xs text-hint">{field.helpAr}</p>}
      {error && (
...
function defaultValues(def: EntityDef): Values {
  ...
  for (const f of def.fields) {
    if (f.type === "enum" && f.required && f.options)
      defaults[f.name] =
        f.name === def.statusField
          ? def.key === "products"
            ...
            : def.key === "leads"
              ...
              : def.key === "policies"
                ? "DRAFT"
                : f.options.includes("ACTIVE")
                  ? "ACTIVE"
                  : f.options[0]
          : f.options.includes("ar")
            ? "ar"
            : undefined;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        {open && <RecordFormBody key={`${def.key}:${existing?._id ?? "new"}`} def={def} onOpenChange={onOpenChange} existing={existing} onSaved={onSaved} />}
      </DialogContent>
    </Dialog>
  );
}
...
function RecordFormBody({ def, onOpenChange, existing, onSaved }: Omit<RecordFormProps, "open">) {
  ...
  function applyError(e: unknown) {
    const data = (
      e as { data?: { code?: string; details?: { errors?: { field: string; message: string }[]; candidates?: typeof duplicates }; message?: string } }
    ).data;
    ...
    <>
      ...
      <form onSubmit={(e) => submit(e)} className="space-y-4">
        ...
        {sections.pricing.length > 0 && (
          <fieldset className="rounded-xl border border-border bg-secondary/40 p-4">
            <legend className="px-1 text-sm font-medium">{t.data.pricingLegend}</legend>
      ...
      <p className="text-xs text-hint">{t.data.stampNote}</p>
    </>
  );
}
