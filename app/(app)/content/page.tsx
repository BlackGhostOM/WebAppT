...
export default function ContentPage() {
  ...
  async function submitContent() {
    const ts = form.date
      ? (fromDateInputValue(form.date) ?? Date.now()) + Number(form.time.split(":")[0]) * 3_600_000 + Number(form.time.split(":")[1] ?? 0) * 60_000
      : Date.now();
    try {
      await createContent({
    ...
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          <>
            <AskAgentButton
              label="اطلب خطة محتوى"
              template="خطط حملة محتوى لأسبوعين على إنستجرام وسناب شات لباقاتنا الفعّالة في موسم الشتاء: 6 منشورات بنصوص عربية قصيرة وأفكار مرئية وهاشتاقات وأوقات نشر، ملتزمة بدليل الهوية، ودون ذكر أسعار إلا من منتج فعّال بصيغة «ابتداءً من»."
            />
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {CAMPAIGN_TRANSITIONS[c.status].map((s) => (
                    <Button
                      ...
                      size="xs"
                      variant="outline"
                      onClick={() => updateCampaign({ campaignId: c._id, status: s }).catch((e) => toast.error(errMsg(e, t.common.error)))}
                    >
                      {labelOf(s, locale)}
                    </Button>
                  ))}
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 md:grid-cols-7">
          {days.map((d) => {
            const items = (content ?? [])
              .filter((c) => c.scheduledAt.timestamp >= d && c.scheduledAt.timestamp < d + DAY)
              .sort((a, b) => a.scheduledAt.timestamp - b.scheduledAt.timestamp);
                ...
                <div className="space-y-1">
                  {items.map((c) => (
                    <button
                      ...
                      className={cn(
                        "block w-full rounded-md border border-border p-1.5 text-start text-xs transition-colors hover:bg-accent/60",
                        c._id === selected && "border-primary/40 bg-primary-soft",
                      )}
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span>{labelOf(c.platform, locale)}</span>
                        <span className="text-muted-foreground">
                          {new Date(c.scheduledAt.timestamp).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", timeZone: "Asia/Muscat" })}
                        </span>
                      </div>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {content?.map((c) => (
              <button
                ...
                onClick={() => setSelected(c._id)}
                className={cn(
                  "rounded-lg border border-border bg-card p-3 text-start text-sm shadow-xs transition-colors hover:bg-accent/60",
                  c._id === selected && "border-primary",
                )}
              >
                ...
                <div className="mt-1 flex items-center justify-between">
                  <StatusBadge value={c.status} />
                  {c.hashtags.length > 0 && <span className="truncate text-xs text-primary-text">{c.hashtags.slice(0, 3).join(" ")}</span>}
                </div>
              </button>
            ))}
          </div>
          ...
          <CardContent className="space-y-3 p-4 text-sm">
            {!current ? (
              ...
              <>
                ...
                <div className="whitespace-pre-wrap rounded-lg bg-secondary p-2">{current.caption}</div>
                {current.captionEn && (
                  <div className="whitespace-pre-wrap rounded-lg bg-secondary p-2 text-xs" dir="ltr">
                    {current.captionEn}
                  </div>
                )}
                {current.hashtags.length > 0 && <div className="text-xs text-primary-text">{current.hashtags.join(" ")}</div>}
                {current.visualIdea && <div className="text-xs text-muted-foreground">🎨 {current.visualIdea}</div>}
                {current.rejectionReason && <div className="text-xs text-destructive-text">سبب الرفض: {current.rejectionReason}</div>}
                {current.externalPostId && (
                  <div className="text-xs text-muted-foreground" dir="ltr">
                    post: {current.externalPostId}
                  </div>
                )}
                {current.approvalId && current.status === "PENDING_APPROVAL" && (
                  <Link href={`/approvals?id=${current.approvalId}`} className="block text-xs text-primary-text underline-offset-4 hover:underline">
                    فتح طلب الاعتماد ←
                  </Link>
                )}
                    ...
                    {CONTENT_TRANSITIONS[current.status]
                      ...
                      .map((s) => (
                        <Button
                          ...
                          onClick={() =>
                            updateStatus({ contentId: current._id, status: s })
                              .then(() => toast.success(labelOf(s, locale)))
                  ...
                  {CONTENT_TRANSITIONS[current.status].includes("REJECTED") && (
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          updateStatus({ contentId: current._id, status: "REJECTED", reason: rejectReason })
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                value={form.platform}
                onValueChange={(v) => setForm({ ...form, platform: String(v ?? "INSTAGRAM") })}
                items={CONTENT_PLATFORMS.map((p) => ({ value: p, label: labelOf(p, locale) }))}
              >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, campaignId: String(v ?? "") })}
                items={(campaigns ?? []).map((c) => ({ value: c._id, label: c.name }))}
              >
                <SelectTrigger className="w-full">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, productId: String(v ?? "") })}
                items={(products ?? []).map((p) => ({ value: String(p._id), label: String(p.name) }))}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="—" />
                </SelectTrigger>
                ...
                {CONTENT_PLATFORMS.map((p) => (
                  <label key={p} className="flex items-center gap-1.5">
                    <Checkbox
                      ...
                      onCheckedChange={(v) =>
                        setCampaignForm({ ...campaignForm, platforms: v ? [...campaignForm.platforms, p] : campaignForm.platforms.filter((x) => x !== p) })
                      }
                    />{" "}
                    {labelOf(p, locale)}
                  </label>
